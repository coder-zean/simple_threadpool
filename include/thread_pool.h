...
public:
    ...
    void AddTask(TClass* t_this, TReturn (TClass::*t_func)(TParam...),
                TParam&&... t_param) {
        {
            ...
            auto task = std::bind(std::forward<decltype(t_func)>(t_func), t_this, std::ref(t_param)...);
    ...
    // 支持类成员函数作为任务添加入线程池
    template<typename TClass, typename TReturn, typename... TParam>
    std::future<TReturn> Submit(TClass* t_this, TReturn (TClass::*t_func)(TParam...),
                TParam&&... t_param) {
        auto task = std::make_shared<std::packaged_task<TReturn()>>(
                std::bind(std::forward<decltype(t_func)>(t_func), t_this, std::ref(t_param)...));
    ...
    // 支持有参的可调用对象作为任务添加入线程池
    template<typename Callable, typename... TParam>
    auto Submit(Callable&& func, TParam&&... t_param) -> std::future<decltype(func(t_param...))> {
        using ReturnType = decltype(func(t_param...));
        auto task = std::make_shared<std::packaged_task<ReturnType()>>(
                std::bind(std::forward<Callable>(func), std::forward<TParam>(t_param)...));
        std::future<ReturnType> result = task->get_future();
        {
            std::lock_guard<std::mutex> locker(mtx_);
            task_list_.emplace([task]() { (*task)(); });
